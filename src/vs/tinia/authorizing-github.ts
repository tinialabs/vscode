...
const CLIENT_ID = '7d0a65abd28a0b19f30f';
...
const AUTH_PAGE_ORIGIN = 'https://auth.tinia.org';
...
export const getGitHubAccessToken = (retryWithOverlay: boolean = false) => {
	...
	return new Promise<AuthMessageData>((resolve) => {
		...
		if (!opener && retryWithOverlay) {
			overlayOpened = true;
			commands.executeCommand('tinia.vscode.get-github-access-token-with-overlay')
		...
		const handleAuthMessage = (event: MessageEvent<{ type: string, payload: AuthMessageData }>) => {
			...
			overlayOpened && commands.executeCommand('tinia.vscode.hide-authorizing-overlay');
