...
import { getGitHubAccessToken, AuthMessageData } from 'vs/tinia/authorizing-github';
...
export class AuthorizingOverlay {
	...
	public show() {
		...
		// add `tinia-overlay-visible` to document.body for `filter: blur(2px)`
		document.body.classList.add('tinia-overlay-visible');
	...
	public hide() {
		...
		setTimeout(() => {
			...
			document.body.classList.remove('tinia-overlay-visible');
	...
	private getDialogElement(): HTMLDivElement {
		return this.getRootElement().querySelector('.tinia-authorizing-dialog')!;
	}
	...
	private createElements(): HTMLDivElement {
		...
		element.classList.add('tinia-authorizing-mask');
	...
	private async handleClickSubmitTokenButton() {
		...
		const tokenStatus = (await commands.executeCommand('tinia.validate-token', token, true)) as ValidateResult;
